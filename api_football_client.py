"""API-Football 数据客户端：为分析模型补充基本面数据（积分榜排名/状态/主客场/净胜球）。免费版100次/天。设计：
- 队名/联赛 ID 用内存缓存，避免重复请求
- standings 按联赛整榜拉取一次，供该联赛所有比赛复用（省额度）
- 用 Bzzoiro 原始英文队名匹配 API-Football 的 team
"""
import os
import logging
import requests
...
def _fetch(path, params):
    """拉取 API-Football。限流(rateLimit)时快速降级返回空，不拖慢主流程。"""
    try:
        ...
        if errors:
            if any('rateLimit' in str(e).lower() or 'limit' in str(e).lower()
                   for e in errors.values() if isinstance(e, str)):
...
def fetch_league_standings(league_cn):
    """按联赛中文名拉取整榜 teams: {name_en: row}。整榜缓存，供同联赛所有比赛复用。"""
    if not _enabled():
        return {}
    if _rate_limited():
        return {}
...
def enrich_form_data(hrow, arow):
    """从 standings 行提取基本面字段，供 _fundamental_pick 增强。返回 (dict, dict) 或 (None, None)。"""
    def extract(row):
        if not row:
            return None
        return {
