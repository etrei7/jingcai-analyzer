...
- **官方首发（Phase 2）**：`lineup_client.py` 用 API-Football `/fixtures/lineups`，**仅开赛前 120 分钟**内、
  **严格匹配**（联赛 + 日期 + 主客双队名）才展示；后台线程拉取 + 10 分钟缓存；受额度/限流保护。
  **只读展示**（阵型/首发），不改方向；命中后 AI 预览标注"官方首发已确认"。接口 `/api/lineup`。
